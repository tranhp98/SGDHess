...
class Trainer(object):
    ...
    def train_step(self, samples, raise_oom=False):
        #print("trainer at iteration", self.iteration)
        self.iteration += 1
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output, gradsH = self.task.train_step(
                        sample=sample,
                        model=self.model,
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                ...
                #print("reach optimizer_step")
                self.task.optimizer_step(
                    self.optimizer, model=self.model, update_num=self.get_num_updates(), gradsH = gradsH
                )
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    # forward and backward
                    loss, sample_size_i, logging_output, names = self.task.train_step(
                        sample=sample,
                        model=self.model,
                        criterion=self.criterion,
                        optimizer=self.optimizer,
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("optimizer"):
                ...
                #print("reach optimizer_step")
                self.task.optimizer_step(
                    self.optimizer, model=self.model, update_num=self.get_num_updates(), names = names
                )
